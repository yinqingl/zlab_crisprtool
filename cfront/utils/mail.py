...
def mail_new_job(request, job):
    ...
    msg_tmp = """You've successfully submitted a job -- ({job_name}) to the CRISPR/guides selection server at crispr.mit.edu. To see its output, you can head over to {host}/job/{job_key} where results from the offtarget scan will be reported as they come in. Results will be kept for 30 days after the initial submission."""
...
def mail_completed_job(request, job):
    ...
    msg_tmp = """Your recent job -- ({job_name}) has run to completion at crispr.mit.edu. To see its output, please head over to http://crispr.mit.edu/job/{job_key}. Results will be kept for 30 days after the initial submission."""
