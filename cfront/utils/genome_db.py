...
def process_hits_for_spacer(spacer_id, hits):
    with transaction.manager:
        ...
        #creat a hash to prevent redundance hits
        hits_dict = {};
        ...
        for idx in hits_taken_idxs:
            ...
            hit_spacerid_chr_start_strand_key = (spacer_id,chr,start,strand)
            if hit_spacerid_chr_start_strand_key in hits_dict:
              continue
            else:
              hits_dict[hit_spacerid_chr_start_strand_key] = 1
        ...
        #FIND EXONS
        if len(spacer.hits)  > 0 and spacer.job.genome_name in ["mm9", "hg19","rn5","danRer7","ce10","dm3","susScr3","oryCun2","monDom5","galGal4","tair10","canFam3","gasAcu1","GRCz10"]:
